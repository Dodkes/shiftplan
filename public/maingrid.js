...
let [dayshift, nightshift, vacation, paragraf, freeshift, remove, shiftleader] =
  falseArray;

function controlPanelReset(id) {
  [dayshift, nightshift, vacation, paragraf, freeshift, remove, shiftleader] =
    falseArray;
    ...
    if (clickedIdSplit[0] === "Operator") {
      if (event.target.id) {
        if (dayshift) {
          ...
          popSLfromSummaryGrid(event.target);
          removeMobileWorkOnRemoveButton(event.target, null);
      ...
      document.getElementById(`line ${clickedIdSplit[1]}`).title =
        updateTooltip(
          operatorsArray[clickedIdSplit[1] - 1],
          operatorsArray[clickedIdSplit[1] - 1].shiftleader
        );
      tooltipRender();
    }
  });
    ...
    for (i = 0; i < operatorsArray.length; i++) {
      ...
      if (columnCell.textContent === shift) {
        if (columnCell.style.background == SLcellColor) {
          ...
          x.title = updateTooltip(
            operatorsArray[b],
            operatorsArray[b].shiftleader
          );
        }
...
function popSLfromSummaryGrid(e) {
  if (
    e.style.background ===
    "linear-gradient(320deg, rgb(252, 208, 61) 0%, rgb(255, 231, 0) 100%)"
  ) {
    let x = splitId(e);
    let y = x[2];
    if (e.textContent === "D") {
      summaryCellUpdate(
        getElementColumn(e, "shiftleaderD"),
        "0.5vw",
        "none",
        white,
        "-"
      ...
      summaryCellUpdate(
        getElementColumn(e, "shiftleaderN"),
        "0.5vw",
        "none",
        white,
        "-"
